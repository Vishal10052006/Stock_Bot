...
M-19 now includes reusable producer-seam wiring for Market Bot, Analysis Bot, and Prediction Inference: each accepts the same MonitoringRuntime boundary and emits telemetry after successful completion. The authoritative Paper Decision Loop remains wired for Strategy/Risk/Execution. A deployment-specific top-level application loop still must pass one shared MonitoringRuntime instance through all producer calls. Monitoring remains observational and never replaces Risk or Independent Safety.
