...
def evaluate_performance_monitoring(snapshot):
    """Report performance telemetry using finite-safe metric values."""
    r, t, c, b = (
        snapshot.returns,
        snapshot.trade_pnls,
        snapshot.equity_curve,
        snapshot.benchmark_returns,
    )
    out = {
        "return_count": len(r),
        "trade_count": len(t),
    ...
    if r:
        ...
        if len(r) > 1:
            ...
            out["volatility"] = math.sqrt(
                sum((x - m) ** 2 for x in r) / (len(r) - 1)
            )

    if t:
        wins = [x for x in t if x > 0]
        losses = [-x for x in t if x < 0]
        out["win_rate"] = len(wins) / len(t)
        # With no losing trades, profit factor is undefined/infinite.
        # Monitoring metrics require finite values, so represent that state
        # explicitly as None rather than emitting math.inf.
        out["profit_factor"] = sum(wins) / sum(losses) if losses else None
        out["expectancy"] = sum(t) / len(t)
    ...
    if b:
        ...
        if r:
            out["active_return_mean"] = (
                out["mean_return"] - out["benchmark_mean_return"]
            )
