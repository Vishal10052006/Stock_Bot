    ...
    def run(self, rows: pd.DataFrame) -> BacktestResult:
        ...
        for _, row in working.iterrows():
            ...
            if strategy.direction is StrategyDirection.NO_TRADE:
                ...
                target_price = self._target_for(candidate.entry_price, candidate.stop_price, candidate.direction)
                risk = self.risk_engine.evaluate(
                    RiskEvaluationInput(
                        candidate=candidate,
                        portfolio=self._portfolio_state(timestamp),
                        market=self._market_context(row, timestamp),
