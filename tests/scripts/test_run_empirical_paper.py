...
from scripts.trading.run_empirical_paper import _load_rows, _load_sidecar, _non_negative_int
...
def test_empirical_loader_requires_preordered_rows(tmp_path: Path) -> None:
    ...
    pd.DataFrame([
        _row("2026-09-21T10:05:00Z"),
        _row("2026-09-21T10:00:00Z"),
    ]).to_parquet(path)
...
def test_non_negative_int_rejects_boolean() -> None:
    with pytest.raises(ValueError, match="integer"):
        _non_negative_int({"events": True}, "events")


def test_non_negative_int_rejects_negative() -> None:
    with pytest.raises(ValueError, match="non-negative"):
        _non_negative_int({"events": -1}, "events")
