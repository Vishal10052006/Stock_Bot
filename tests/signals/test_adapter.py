...
from trading.signals.adapter import build_candidate_from_decision
...
def make_decision(direction: StrategyDirection) -> StrategyDecision:
...
def test_long_decision_becomes_long_candidate() -> None:
    candidate = build_candidate_from_decision(make_decision(StrategyDirection.LONG), make_row())
...
def test_short_decision_becomes_short_candidate() -> None:
    candidate = build_candidate_from_decision(make_decision(StrategyDirection.SHORT), make_row())
    assert candidate is not None
...
def test_timestamp_mismatch_is_rejected() -> None:
    decision = make_decision(StrategyDirection.LONG)
    row = make_row()
    row["timestamp"] = pd.Timestamp("2026-09-15 10:05:00+05:30")

    import pytest

    with pytest.raises(ValueError, match="timestamp does not match"):
        build_candidate_from_decision(decision, row)
